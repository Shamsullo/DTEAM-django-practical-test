...
class ContactAPITests(TestCase):
    def setUp(self):
        ...
        # URLs
        self.contact_list_url = reverse('cv-contacts-list',
                                        kwargs={'cv_pk': self.cv.pk})
        self.contact_detail_url = reverse('cv-contacts-detail',
                                          kwargs={'cv_pk': self.cv.pk,
                                                  'pk': self.contact.pk})
    ...
    def test_update_contact(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(Contact.objects.get(id=self.contact.id).value,
                         '+9924567890')
